...
from pg_base.select_pg import get_price_by_week
import os
import subprocess
import pandas as pd
import numpy as np
...
def to_xlxs(frame):
    ...
    with pd.ExcelWriter(file_path, engine='xlsxwriter') as writer:
        ...
        # Устанавливаем формат для столбцов 3–5 (column3, column4, column5)
        number_format = workbook.add_format({'num_format': '# ##0.00000000'})  # Формат числа с 6 знаками после запятой
...
if __name__ == '__main__':
    ...
    day_of_week = 2  # 0 - Понедельник, 1 - Вторник, ..., 6 - Воскресенье
    ...
    for i in range(0, weeks_between+1):
        ...
        response_frame = get_price_by_week(i, ind_week)
    ...
    merged_df = merged_df.groupby(['day_of_week', 'hour', 'minute'], as_index=False).mean(numeric_only=True)
    ...
    merged_df['итог_mean'] = merged_df[columns_to].mean(axis=1)
    merged_df['итог_median'] = merged_df[columns_to].median(axis=1)
    merged_df['итог_std'] = merged_df[columns_to].std(axis=1)
    cnt = 3
    ...
    for h in alphas:
        ...
        merged_df[f'итог_ewn_aplha_{h}'] = merged_df[columns_to].quantile(round(h, 3), axis=1)
        cnt += 1
        #
        # .rolling(window=3)
